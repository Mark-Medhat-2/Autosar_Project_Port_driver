...
void Port_SetupPin(const Port_ConfigChannel * ConfigPtr )
{
    ...
    switch(ConfigPtr->mode)
    {
      case ANALOG: SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , ConfigPtr->pin_num);        /* Set the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
                   ...
                   CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , ConfigPtr->pin_num);        /* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
	...
	case  3: PortGpio_Ptr = (volatile uint32 *)GPIO_PORTD_BASE_ADDRESS; /* PORTD Base Address */
        ...
        case  5: PortGpio_Ptr = (volatile uint32 *)GPIO_PORTF_BASE_ADDRESS; /* PORTF Base Address */
		 break;
            ...
            if(Port_PortChannels[Pin].direction == PORT_PIN_OUT && Port_PortChannels[Pin].mode_changeable==STD_OFF )
          {
              SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) ,Port_PortChannels[Pin].pin_num);                /* Set the corresponding bit in the GPIODIR register to configure it as output pin */
              
          }
          else if(Port_PortChannels[Pin].direction == PORT_PIN_IN && Port_PortChannels[Pin].mode_changeable==STD_OFF)
          {
              CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_PortChannels[Pin].pin_num);             /* Clear the corresponding bit in the GPIODIR register to configure it as input pin */
...
void Port_SetPinMode(Port_PinType Pin, Port_PinModeType Mode ){
   volatile uint32 * PortGpio_Ptr = NULL_PTR;                          /* point to the required Port Registers base address */
	...
	if (Port_Status == PORT_NOT_INITIALIZED)
	{
		Det_ReportError(PORT_MODULE_ID, PORT_INSTANCE_ID, PORT_SET_PIN_MODE_ID,
		PORT_E_UNINIT);
                error = TRUE;
	}
	else
        ...
        else
	{
       ...
       if(Port_PortChannels[Pin].mode_changeable == STD_OFF)
        ...
        if(Port_PortChannels[Pin].mode != GPIO ||Port_PortChannels[Pin].mode != mode1 ||Port_PortChannels[Pin].mode != mode2 ||Port_PortChannels[Pin].mode != mode3 ||
           Port_PortChannels[Pin].mode != mode4 ||Port_PortChannels[Pin].mode != mode1 ||Port_PortChannels[Pin].mode != mode5 ||Port_PortChannels[Pin].mode != mode6 ||
           Port_PortChannels[Pin].mode != mode7 ||Port_PortChannels[Pin].mode != mode8 ||Port_PortChannels[Pin].mode != mode9 ||Port_PortChannels[Pin].mode != mode14 ||
           Port_PortChannels[Pin].mode != ANALOG)
        {
          Det_ReportError(PORT_MODULE_ID, PORT_INSTANCE_ID, PORT_SET_PIN_MODE_ID,
  ...
  if(error==FALSE){
            ...
            switch(Mode)
    {
      case ANALOG: SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , Port_PortChannels[Pin].pin_num);        /* Set the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
                   ...
                   CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , Port_PortChannels[Pin].pin_num);        /* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
